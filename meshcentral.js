...
function CreateMeshCentralServer(config, args) {
    ...
    obj.StartEx = async function () {
        ...
        if (agentEnrolmentPolicy.isResolvableIpList(config.settings.agentallowedipnewagents)) { config.settings.agentallowedipnewagents = await resolveDomainsToIps(config.settings.agentallowedipnewagents); }
    ...
    obj.StartEx1b = async function () {
        ...
        for (i in obj.config.domains) {
            ...
            if (agentEnrolmentPolicy.isResolvableIpList(obj.config.domains[i].agentallowedipnewagents)) { obj.config.domains[i].agentallowedipnewagents = await resolveDomainsToIps(obj.config.domains[i].agentallowedipnewagents); }
    ...
    function readIpListFromFile(arg) {
        return agentEnrolmentPolicy.readIpListFromFile(arg, obj.datapath);
    }
